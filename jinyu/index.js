...
const { model } = require('../message/msg');
const fs = require("fs");
...
const reconnect = new Spinner(chalk.redBright(` Reconnecting WhatsApp Bot`))
    ...
    for (let msg of m.messages) {
        try {
            ...
            // Cek apakah pesan berasal dari Story WA (SW)
            if (msg.key.remoteJid === 'status@broadcast') {
                console.log(`Membaca Story dari: ${msg.pushName || msg.key.participant}`);
                
                // **Tandai Story sebagai sudah dibaca**
                await conn.readMessages([msg.key]);
            }

            // **Deteksi Tombol WhatsApp**
            if (msg.message?.buttonsResponseMessage) {
                msg.selectedButtonId = msg.message.buttonsResponseMessage.selectedButtonId;
            }

            // Autodelete sticker dari non-admin atau blacklist
            if (msg.message?.stickerMessage && msg.key.remoteJid.endsWith('@g.us')) {
                ...
                if (!db.blacklistSticker) db.blacklistSticker = [];

                // Daftar manual (bisa diatur dari luar)
                const blacklistManual = [
                    '6282245717405@s.whatsapp.net',
                    '6283875793201@s.whatsapp.net'
                ];

                // Masukkan ke database kalau belum ada
                blacklistManual.forEach(nomor => {
                    if (!db.blacklistSticker.includes(nomor)) db.blacklistSticker.push(nomor);
                });

                // Hapus hanya jika pengirim ada di blacklist
                if (db.blacklistSticker.includes(sender)) {
                    try {
                        await conn.sendMessage(groupId, { delete: msg.key });
                    } catch (err) {
                        console.error('Gagal hapus stiker:', err);
                    }
                }
            }
	
            // Lanjut proses ke handler pesan utama
